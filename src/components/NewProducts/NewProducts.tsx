...
export function NewProducts({ productData }: NewProductsProps) {
    ...
    const [wishlistProduct, setWishlistProduct] = useState<ProductDataType | null>(null);
    ...
    return (
        <div className='Product-left-new-products pt-10'>
            <div className='flex justify-between items-center mb-6'>
                ...
                <Link href="/shop" className='text-black hover:text-black flex items-center gap-1'>
                    <Button
                        variant="ghost"
            ...
            <div className='grid grid-cols-4 gap-6'>
                {productData.slice(0, 8).map((product) => (
                    <div key={product.id} className="cursor-pointer group relative h-full">
                            ...
                            <div className="flex flex-col justify-between flex-grow">
                                <div className="space-y-1.5">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-gray-400 line-through text-sm">${(Number(product.price) * 1.3).toFixed(2)}</span>
                                        <span className="text-[#D51243] text-lg font-bold">${Number(product.price).toFixed(2)}</span>
                                    </div>
                                </div>
                                    ...
                                    {selectedQuantities[product.id] ? (
                                        <div className="flex items-center justify-between bg-gray-100 rounded-full p-1">
                                            <Button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    handleQuantityChange(product.id, selectedQuantities[product.id] - 1);
                                                }}
                                                size="icon"
                                                variant="secondary"
                    ...
                    {selectedProduct && (
                        <div className="grid grid-cols-2 gap-6">
                            ...
                            <div className="p-6 pr-8">
                                ...
                                {/* Product Title */}
                                <h2 className="text-2xl font-bold text-gray-900 mb-4">{selectedProduct.title}</h2>
                                    ...
                                    {selectedQuantities[selectedProduct.id] ? (
                                        <div className="flex items-center justify-between bg-gray-100 rounded-full p-1 w-[200px]">
                                            <Button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    handleQuantityChange(selectedProduct.id, selectedQuantities[selectedProduct.id] - 1);
                                                }}
                                                size="icon"
                                                variant="secondary"
                ...
                <DialogContent className="sm:max-w-[500px] text-center p-6">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="space-y-2">
                            <h3 className="text-xl font-semibold">Successfully added to Wishlist</h3>
                            {wishlistProduct && (
